...
overflow-y:scroll; background: ghostwhite;}"))
  ...
  shiny::fluidRow(
    ...
    shinydashboard::box(
      ...
      shiny::actionButton(inputId = "build_gene_plot", "Generate plot",
...
server <- function(input, output, session) {
  ...
  plot_df <- reactive({
    ...
    gene_seg_data_server$data() |>
      dplyr::filter(.data[[input$population_var_gene]] %in% input$pop_to_plot)


  })

  # create a a dataframe to plot

  shiny::observeEvent(input$build_gene_plot, {
    output$dist_plot <- shiny::renderPlot({
      ...
      theme. <- ggplot2::theme_light()  +
        ggplot2::theme(legend.position = "bottom", text = ggplot2::element_text(colour = "black", size = 18))


      #histogram plot
      if (input$plot_type_gene == "histogram") {
        plot_df() |>
          ggplot2::ggplot(ggplot2::aes_string(x = input$phenotype_var_gene, fill = input$population_var_gene)) +
          ggplot2::geom_histogram(color="#e9ecef", alpha=0.6, position = 'identity', bins = input$bins_hist) +
          theme. +
          ggplot2::scale_fill_manual(values = RColorBrewer::brewer.pal(length(input$pop_to_plot),"Dark2"))
      } else {
        # density
        if (input$plot_type_gene == "density") {
          plot_df() |>
            ggplot2::ggplot(ggplot2::aes_string(input$phenotype_var_gene, fill = input$population_var_gene)) +
            ggplot2::geom_density(color="#e9ecef", alpha=0.6) +
            theme. +
            ggplot2::scale_fill_manual(values = RColorBrewer::brewer.pal(length(input$pop_to_plot),"Dark2"))
        } else {
          #ridge histogram
          if(input$plot_type_gene == "ridge histogram") {
            plot_df() |>
              ggplot2::ggplot(ggplot2::aes_string(x = input$phenotype_var_gene, y = input$population_var_gene,
                                                  fill = input$population_var_gene)) +
              ggridges::geom_density_ridges(alpha = 0.6, stat="binline", bins = input$bins_hist)+
              theme. +
              ggplot2::scale_fill_manual(values = RColorBrewer::brewer.pal(length(input$pop_to_plot),"Dark2"))
          } else {
            #ridge density
            if(input$plot_type_gene == "ridge density") {
              plot_df() |>
                ggplot2::ggplot(ggplot2::aes_string(x = input$phenotype_var_gene, y = input$population_var_gene,
                                                    fill = input$population_var_gene)) +
                ggridges::geom_density_ridges(alpha = 0.6, bandwidth = 0.95) +
                theme. +
                ggplot2::scale_fill_manual(values = RColorBrewer::brewer.pal(length(input$pop_to_plot),"Dark2"))
            }
